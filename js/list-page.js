    ...
    const employeeManagement = {
        ...
        handleSearch: (searchTerm) => {
            ...
            const filteredEmployees = state.allEmployees.filter(employee => 
                ...
                (employee.EmployeeId && employee.EmployeeId.toString().toLowerCase().includes(lowerCaseSearchTerm))
            );
            state.currentPage = 1;
        ...
        displayEmployees: (employees, page = 1) => {
            ...
            paginatedEmployees.forEach((employee, index) => {
                ...
                row.innerHTML = `
                    ...
                    <td>
                        <button class="edit-btn" data-id="${employee.EmployeeId}">Sửa</button>
                        <button class="toggle-btn" data-id="${employee.EmployeeId}">
        ...
        fillEmployeeModal: (employee) => {
            document.querySelector('input[name="employeeCode"]').value = employee.EmployeeCode || '';
            document.querySelector('input[name="fullName"]').value = employee.FullName || '';
            document.querySelector('input[name="position"]').value = employee.PositionName || '';
            document.querySelector('input[name="department"]').value = employee.DepartmentName || '';
            document.querySelector('input[name="dateOfBirth"]').value = employee.DateOfBirth ? new Date(employee.DateOfBirth).toISOString().split('T')[0] : '';
            ...
            document.querySelector('input[name="identityDate"]').value = employee.IdentityDate ? new Date(employee.IdentityDate).toISOString().split('T')[0] : '';
            ...
            document.querySelector('input[name="mobilePhone"]').value = employee.PhoneNumber || '';
            document.querySelector('input[name="homePhone"]').value = employee.TelephoneNumber || '';
            document.querySelector('input[name="email"]').value = employee.Email || '';
            document.querySelector('input[name="bankAccount"]').value = employee.BankAccountNumber || '';
            document.querySelector('input[name="bankName"]').value = employee.BankName || '';
            document.querySelector('input[name="bankBranch"]').value = employee.BankBranchName || '';
        },
        handleEditClick: async (e) => {
            const editBtn = e.target.closest('.edit-btn');
            if (editBtn) {
                const employeeId = editBtn.dataset.id;
                const employeeDetails = await employeeManagement.getEmployeeDetails(employeeId);
                if (employeeDetails) {
        ...
        saveEmployeeChanges: async () => {
            ...
            try {
                const response = await fetch(`https://cukcuk.manhnv.net/api/v1/Employees/${employeeData.EmployeeCode}`, {
                    method: 'PUT',
                    headers: {
        ...
        const ui = {
            openModal: (mode = 'add') => {
                ...
                if (mode === 'add') {
                    document.querySelector('.modal-title').textContent = 'Thêm mới nhân viên';
                    document.querySelector('.modal .btn:not(.outline)').textContent = 'Thêm mới';
                } else {
                    document.querySelector('.modal-title').textContent = 'Chỉnh sửa thông tin nhân viên';
                    document.querySelector('.modal .btn:not(.outline)').textContent = 'Lưu';
                }
            },
            ...
            updatePagination: (totalItems, currentPage) => {
                ...
                let paginationHTML = `
                    ...
                    <div class="row-per-page">
                        <span class="value">${state.itemsPerPage} bản ghi trên 1 trang</span>
        ...
        window.changePage = function(newPage) {
            if (newPage < 1 || newPage > Math.ceil(state.allEmployees.length / state.itemsPerPage)) return;
        ...
        domElements.saveBtn.onclick = () => {
            const mode = document.querySelector('.modal-title').textContent.includes('Thêm mới') ? 'add' : 'edit';
            if (mode === 'add') {
                // Xử lý logic thêm mới nhân viên
                ui.showToast('Đã thêm mới nhân viên thành công.');
            } else {
                // Xử lý logic cập nhật thông tin nhân viên
                ui.showToast('Đã cập nhật thông tin nhân viên thành công.');
            }
            ui.closeModal();
        };
        ...
        domElements.table.addEventListener('click', employeeManagement.handleEditClick);
